...
result <- data.frame(array(NA, dim=c(len_omics,11)))
names(result) <- c('Category','Phenotype','HR','z','P','N_total','N_mdd','Chisq','Chisq_P','G_Chisq','G_Chisq_P')
...
for(i in 1:len_omics){
  ...
  result[i,10] <- PH_test["GLOBAL","chisq"]
  result[i,11] <- PH_test["GLOBAL","p"]
